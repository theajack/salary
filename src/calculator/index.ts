...
export function calculateSalary ({
    ...
    for (let i = 1; i < 13; i++) {
        ...
        const personalIncomeTax = countPersionalIncomeTax({
            accumulatedTaxableIncome,
            totalPersonalTncomeTax
        }); // 当月个人所得税

        const afterTaxIncome = salary - result.insuranceAndFund.totalFund - personalIncomeTax;
...
// 五险一金计算器
function calculateInsuranceAndFund (base: number, rate) {
    const result = {};
    let totalFund = 0;
